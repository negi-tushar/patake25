...
import {
  ...
  BehaviorSubject,
  combineLatest,
} from 'rxjs'; // Import more from RxJS
...
public categoryControl = new FormControl('');
  ...
  // --- Inline Edit State ---
  public editedPrices = new Map<string, number>(); // Tracks new price for a given product ID
  public savingRowId = signal<string | null>(null); // Tracks which row is currently saving
...
this.products$ = combineLatest([
  ...
  canApplyMargin(): boolean {
    if (
      this.bulkMarginPercent == null ||
      isNaN(this.bulkMarginPercent) ||
      this.bulkMarginPercent < 0
    )
      return false;
  ...
  async applyMargin(): Promise<void> {
    try {
      ...
      this.toast(
        `Applied ${percent}% margin to ${updates.length} product(s).`,
        'success'
      );
    } catch (err) {
  ...
  async saveInlineChange(product: Product): Promise<void> {
    const newPrice = this.editedPrices.get(product.id!);
    ...
    try {
      ...
      this.toast('Failed to update price.', 'error');
    } finally {
